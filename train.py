        ...
        if epoch>args.lr_decrease:
            ...
            if args.update_features:
                ...
                optim_params.append({'params': ie_parameters,
                                 'lr': lrg})
            optimizer = optim.Adam(optim_params, lr=lr, weight_decay=args.wd)
        if epoch>args.warming_epoch:
            mode = 'train'

        train(epoch, image_extractor, model, trainloader, optimizer, writer, mode)
...
def train_normal(epoch, image_extractor, model, trainloader, optimizer, writer,mode='warming_up'):
    '''
    ...
    for idx, data in tqdm(enumerate(trainloader), total=len(trainloader), desc = 'Training'):
        ...
        if mode == 'warming_up':
            loss, _ = model(data,'warming_up')
        else:
            loss, _ = model(data, 'train')
